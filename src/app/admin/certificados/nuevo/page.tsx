...
export default function NewCertificatePage() {
  ...
  const [successData, setSuccessData] = useState<CertificateResponse | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "Error al crear certificado");
      }
  ...
  const downloadQR = async () => {
    ...
    try {
      ...
      link.download = `QR-${successData.folio}.png`;
      link.href = canvas.toDataURL("image/png");
      link.click();
    } catch (downloadError) {
      console.error("Error generating QR:", downloadError);
      alert("Error al generar la etiqueta QR. Por favor, intente de nuevo.");
    }
  };
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Constancia-EHSW-${successData.folio}.pdf`);
    } catch (downloadError) {
    ...
    return (
      <div className="flex flex-col lg:flex-row min-h-screen">
        ...
        <main className="flex-1 p-6 lg:p-10 flex flex-col items-center">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
              <div>
                ...
                <p className="text-[var(--color-text-muted)] text-sm mt-1">
                  El certificado y la etiqueta QR ya están listos para descargar.
                </p>
              </div>
              <div className="flex flex-wrap gap-3 w-full md:w-auto">
                ...
                <button
                  ...
                  className="btn-primary text-sm flex-1 md:flex-none justify-center"
                >
          ...
          <div
            id="certificado-view"
            className="bg-white rounded-xl shadow-2xl overflow-hidden mb-10 text-slate-800 w-full max-w-4xl"
          >
              ...
              <div className="grid grid-cols-2 gap-x-20 gap-y-6 mt-12 text-left z-10 w-full max-w-3xl">
                ...
                <div>
                  ...
                  <p className="text-lg text-slate-800">
                    {new Date(successData.issueDate).toLocaleDateString("es-MX")}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase text-slate-500 font-bold tracking-wider mb-1">
                  ...
                  <p className="text-lg text-slate-800">
                    {new Date(successData.expirationDate).toLocaleDateString("es-MX")}
                  </p>
                </div>
              </div>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="mb-8">
            <Link
              ...
              className="text-[var(--color-primary)] no-underline hover:underline text-sm mb-4 inline-block font-medium"
            >
              ← Volver a la lista
            </Link>
            <h1 className="text-2xl font-bold text-white">Nuevo Certificado</h1>
            <p className="text-[var(--color-text-muted)] text-sm">
              Complete la información para generar el certificado y su código QR.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="glass-card p-8 space-y-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, chemicalUsed: e.target.value })
                }
                className="contact-input"
